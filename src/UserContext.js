...
export const UserProvider = ({ children }) => { // UserProvider component is created and can be exported to other components, the children prop represents any elements or components that are within the UserProvider component
  const [user, setUser] = useState(null); //  user value is meant to store user data(set to null value by default), setUser function is present to update user value depending on scenario


  useEffect(() => {
    axios.get('http://localhost:3001/api/user') 
      .then((response) => {
        setUser(response.data); // Update the user context with the fetched data from database
        console.log('User Data:', response.data); // User data is logged to the console with the response
      })
